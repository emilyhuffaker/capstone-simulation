...
# Read condition values from CHTC
args <- commandArgs(trailingOnly = TRUE)

causal_effect <- as.numeric(args[1])
med1_loading <- as.numeric(args[2])
med2_loading <- as.numeric(args[3])
...
# Store results from 500 samples
...
# Create unique output filename for this condition
output_file <- paste0(
  "latentvariable_summary_ce",
  causal_effect,
  "_m1",
  med1_loading,
  "_m2",
...
# Write only the one-row summary
write_csv(
  condition_summary,
  output_file
)
